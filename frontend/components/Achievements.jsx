...
import { Trophy, Flame, Star, Award, XCircle, ChevronUp, ChevronDown } from "lucide-react"
...
export default function DewordleAchievements() {
  ...
  const badges = [
    { id: "first-win", name: "First Win", icon: Trophy, description: "Win your first game", earned: true },
    { id: "5-day-streak", name: "5-Day Streak", icon: Flame, description: "Play for 5 consecutive days", earned: true },
    {
      id: "flawless-victory",
    ...
    { id: "10-wins", name: "10 Wins", icon: Award, description: "Win 10 games in total", earned: false },
  ]
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
        <h1 className="text-2xl font-bold text-center text-gray-900 dark:text-white mb-6">Dewordle Achievements</h1>

        {/* Streak and Stats */}
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center">
            <Flame className="w-6 h-6 text-orange-500 mr-2" />
            <span className="text-xl font-bold text-gray-800 dark:text-white">{streak} Day Streak</span>
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400">Total Wins: {totalWins}</div>
        </div>
        ...
        {/* Badges Section */}
        <div className="mb-6">
          <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowBadges(!showBadges)}>
            <h2 className="text-lg font-semibold text-gray-800 dark:text-white">Badges</h2>
            {showBadges ? (
              ...
              {badges.map((badge) => (
                <div
                  ...
                  className={`flex items-center p-2 rounded-lg ${
                    badge.earned ? "bg-green-100 dark:bg-green-900" : "bg-gray-100 dark:bg-gray-700 opacity-50"
                  }`}
                >
                  <badge.icon
                    className={`w-8 h-8 mr-2 ${
                      badge.earned ? "text-green-600 dark:text-green-400" : "text-gray-400 dark:text-gray-500"
                    }`}
                  />
                  <div>
      ...
      {showAchievement && latestAchievement && (
        <div className="fixed bottom-4 right-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 flex items-center space-x-3 animate-fade-in-up">
          ...
          <div>
            <p className="font-semibold text-gray-800 dark:text-white">{latestAchievement.name}</p>
